...
/**
 * @namespace Core.UI.RichTextEditorDFTA
 * @memberof Core.UI
 * @author OTRS AG
 * @description
 *      Richtext Editor.
 */
Core.UI.RichTextEditorDFTA = (function (TargetNS) {
    /**
     * @private
     * @name $FormID
     * @memberof Core.UI.RichTextEditorDFTA
     * @member {jQueryObject}
     * @description
    ...
    TargetNS.InitEditor = function ($EditorArea) {
        ...
        if (EditorID === '') {
            Core.Exception.Throw('RichTextEditorDFTA: Need exactly one EditorArea!', 'TypeError');
        }
        ...
        EditorConfig = {
            ...
            removePlugins: CheckFormID($EditorArea).length ? 'image,exportpdf' : 'image,uploadimage,exportpdf',
    ...
    TargetNS.InitAllEditors = function () {
        ...
        $("textarea[id$=RichText]").each(function () {
          //CKEDITOR.replace( $(this).attr("id"), TextAreaCkeditorConfig );
        });
